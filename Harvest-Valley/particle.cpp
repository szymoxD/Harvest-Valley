...
particle::particle(float x, float y, float z, bool type_v, bool dead_v, float speed_v, float acceleration_v, float lifetime, float size_v, float red_v, float green_v, float blue_v) {
	...
	if (type) { //dym
		...
		pos.x+= (float(rand()) / RAND_MAX - 0.5f) * 2 * 27.0f;
		pos.z+= (float(rand()) / RAND_MAX - 0.5f) * 2 * 27.0f;
		speed = speed_v + (float(rand()) / RAND_MAX - 0.5f) * 2 * 8.0f;
		speedx = (float(rand()) / RAND_MAX - 0.5f) * 2 * 50.0f;
		speedz = (float(rand()) / RAND_MAX - 0.5f) * 2 * 50.0f;
		v_acc = (float(rand()) / RAND_MAX);
		angle = (float(rand()) / RAND_MAX - 0.5f) * 2 * 180.0f;
		v_angle = (float(rand()) / RAND_MAX) * 360.0f * 3.14159 / 180.0f;
	}
